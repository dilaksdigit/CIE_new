...
BEGIN
  ...
  IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    ...
    ALTER TABLE audit_log ADD COLUMN actor_id VARCHAR(100) NULL;
  END IF;
  -- actor_role
  IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
      WHERE TABLE_SCHEMA = DATABASE() AND TABLE_NAME = 'audit_log' AND COLUMN_NAME = 'actor_role') = 0 THEN
    ALTER TABLE audit_log ADD COLUMN actor_role VARCHAR(30) NULL;
  END IF;
  -- actor_ip
  IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
  ...
  IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
    ...
    ALTER TABLE audit_log ADD COLUMN actor_user_agent TEXT NULL;
  END IF;
  -- changed_at
  IF (SELECT COUNT(*) FROM INFORMATION_SCHEMA.COLUMNS
