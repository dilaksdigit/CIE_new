...
SELECT gen_random_uuid(), 1, 'problem_solving', 'Problem-Solving', 'User has a problem, needs product to solve it', '["hero","support","harvest"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 1);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 2, 'comparison', 'Comparison', 'User evaluating alternatives', '["hero","support"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 2);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 3, 'compatibility', 'Compatibility', 'User confirming fit with existing setup', '["hero","support","harvest"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 3);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 4, 'specification', 'Specification', 'User needs technical details', '["hero","support","harvest"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 4);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 5, 'installation', 'Installation / How-To', 'User needs help installing or setting up', '["hero","support"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 5);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 6, 'troubleshooting', 'Troubleshooting', 'User diagnosing or fixing a fault or poor performance', '["hero","support"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 6);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 7, 'inspiration', 'Inspiration / Style', 'User browsing for ideas and style guidance', '["hero","support"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 7);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 8, 'regulatory', 'Regulatory / Safety', 'User needs safety, compliance, or regulatory detail', '["hero","support"]'
WHERE NOT EXISTS (SELECT 1 FROM intent_taxonomy WHERE intent_id = 8);

INSERT INTO intent_taxonomy (id, intent_id, intent_key, label, definition, tier_access)
SELECT gen_random_uuid(), 9, 'replacement', 'Replacement / Refill', 'User needs a replacement part or consumable', '["hero","support"]'
