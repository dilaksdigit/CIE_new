...
-- ---------------------------------------------------------------------------
-- 7. BLB-LED-B22-8W (Support) — INTENTIONAL G5 FAIL (not_for empty, minimum 1 required) → G5 FAIL CIE_G5_BESTFOR_COUNT
-- ---------------------------------------------------------------------------
UPDATE skus SET
...
VALUES
  ...
  (UUID(), 'SHD-GLS-CNE-20',
   ...
   'An opal glass cone shade delivers brighter, more focused light than fabric alternatives, making it ideal for kitchen pendants and reading nooks. The 20cm diameter suits compact pendants. Opal finish softens harshness.',
...
-- ---------------------------------------------------------------------------
-- 11b. Set validation_status so Writer Queue shows 2 todo (intentional fails) and 6 done (passing)
--      SHD-GLS-CNE-20 = G4 FAIL; BLB-LED-B22-8W = G5 FAIL. Rest (hero+support) remain VALID from 006.
-- ---------------------------------------------------------------------------
UPDATE skus SET validation_status = 'INVALID'
WHERE sku_code IN ('SHD-GLS-CNE-20', 'BLB-LED-B22-8W');

UPDATE skus SET validation_status = 'VALID'
WHERE sku_code IN ('CBL-BLK-3C-1M', 'CBL-GLD-3C-1M', 'CBL-WHT-2C-3M', 'SHD-TPE-DRM-35', 'BLB-LED-E27-4W', 'PND-SET-BRS-3L');
