...
<!-- SOURCE: CIE_v2.3.1_Enforcement_Dev_Spec §2.1 The Eight Publish Gates -->
<!-- SOURCE: CIE_Master_Developer_Build_Spec §7 Gate table -->
<!-- SOURCE: CIE_v2.3_Enforcement_Edition §1.1 The Seven Publish Gates -->
<!-- Updated to align with authoritative spec definitions (March 2026) -->
...
| G3 | Secondary Intent(s) | Min 1, max 3 from locked taxonomy. Must differ from Primary. | NONE |
| G4 | Answer Block | 250–300 characters (configurable via BusinessRules). Must contain primary intent keyword. | NONE |
| G5 | Best-For / Not-For | Min 2 best_for + min 1 not_for (Hero/Support only). Harvest/Kill = suspended. | NONE |
| G6 | Tier Tag | SKU tier must be one of: hero, support, harvest, kill. Tier is auto-calculated from ERP data. Read-only in Content Writer UI. | NONE |
| G6.1 | Tier-Locked Intents | Kill = ALL content fields disabled. Harvest = Specification + 1 other only. Fields REMOVED from UI, not greyed out. | NONE |
| G7 | Expert Authority | Hero + Support SKUs require min 1 credentialed authority statement. Must reference a specific standard or certification. Harvest/Kill = suspended. | NONE |
| VEC | Vector Similarity | Cosine similarity ≥ 0.72 between description and cluster intent. **Fail-soft**: below threshold = warning, not block. | NONE (warn only) |

**Note:** Channel readiness scoring (0-100 per channel) is enforced at deployment time, not as a publish gate. Hero SKUs require ≥85 on primary channel and ≥70 on all active channels. Thresholds configurable via BusinessRules (`readiness.hero_primary_channel_min`, `readiness.hero_all_channels_min`).
